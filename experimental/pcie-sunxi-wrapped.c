...
static int dw_pcie_iatu_off(struct dw_pcie_rp *pp)
{
	...
	resource_list_for_each_entry(entry, &pp->bridge->windows) {
		...
		ret = dw_pcie_prog_outbound_atu(pci, 0x0, PCIE_ATU_TYPE_MEM,
						entry->res->start,
	...
	if (pp->io_size) {
			ret = dw_pcie_prog_outbound_atu(pci, 0x2, PCIE_ATU_TYPE_IO,
							pp->io_base,
							pp->io_bus_addr,
							pp->io_size);
			if (ret) {
			       ...
			       struct platform_device *pdev)
{
	...
	if (IS_ENABLED(CONFIG_PCI_MSI)) {
		...
		ret = devm_request_irq(&pdev->dev, pp->msi_irq[0],
